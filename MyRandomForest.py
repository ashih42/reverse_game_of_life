...
class MyRandomForest:
	...
	def train(self, filename):
		...
		for i in range(5):
			...
			print('X_delta_i shape = ', X_delta_i.shape, ', Y_delta_i shape = ', Y_delta_i.shape)

			model = RandomForestClassifier(n_estimators=100, random_state=0, verbose=2)
			model.fit(X_delta_i, Y_delta_i.flatten())
			self.models.append(model)
			
		# predictions = self.__predict(X)

		# accuracy = self.__get_accuracy(predictions, Y)
		# f1 = self.__get_f1_score(predictions, Y)
		# print()
		...
		# self.models = []
		# for i in range(5):
		# 	print('Training model for delta = %d' % (i + 1))
		...
		# print('Training...')
		# self.model = RandomForestClassifier(verbose=2)
		# self.model.fit(X, Y)
		# print()
	...
	def __predict(self, X):
		...
		for i in range(5):
			delta = i + 1
			print('Predicting: delta = ', delta)
			row_indices = X[:, 0] == delta

			temp = self.models[i].predict( X[ row_indices, 1: ] )
			predictions[ row_indices ] = temp[:, np.newaxis]
	...
	def predict(self, filename):
		...
		# self.__write_predictions_to_file(predictions.reshape(-1, 400))
	...
	def __wrangle_data(self, data, is_training_data):
		...
		# 	delta = X[i, 0]
		# 	X_new[i, int(delta) - 1] = 1

		# print('finished formatting X_new, shape = ', X_new.shape)
