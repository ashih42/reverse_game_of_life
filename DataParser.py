...
class DataParser:
	...
	def __parse_delta(self, row_data, expr):
		try:
			...
			raise ParserException('Invalid delta at ' +
				Fore.GREEN + 'line %d' % (self.__line_number) + Fore.RESET + ': ' + Fore.MAGENTA + expr + Fore.RESET)
		if not (1 <= delta <= 5):
			raise ParserException('Invalid delta at ' +
				Fore.GREEN + 'line %d' % (self.__line_number) + Fore.RESET + ': ' + Fore.MAGENTA + expr + Fore.RESET)
	...
	def __parse_cell(self, row_data, expr, column_index):
		if expr == '0':
			...
			raise ParserException('Invalid cell value at ' +
				Fore.GREEN + 'line %d, column %d' % (self.__line_number, column_index + 1) + Fore.RESET + ': ' + Fore.MAGENTA + expr + Fore.RESET)
